...
import { getTodayDate, shiftDate, minutesSinceMidnightET, minutesToClock, meetingCutoffMinutes, niceTimeTicks, computeLongestStreak, computeCurrentStreak } from '../lib/dateUtils';
...
export default function HistoryPage() {
  ...
  const [topMeetingId, setTopMeetingId] = useState('');
  ...
  async function loadTopAttendees(timeframe: Timeframe, meetingId: string) {
    setTopLoading(true);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { loadTopAttendees(topTimeframe, topMeetingId); }, [topTimeframe, topMeetingId]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="history-grid">
        ...
        <section className="history-section leaderboard-section">
          <div className="section-header-row">
            <h2>Top Attendees</h2>
            <div className="section-header-controls">
              <RoleMeetingSelect value={topMeetingId} onChange={setTopMeetingId} meetings={meetings} />
              <div className="timeframe-pills">
                {(['4w', '12w', '6m', '1y', 'all'] as const).map(tf => (
                  <button
                    key={tf}
